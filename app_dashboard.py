...
def display_plot_and_explanation(option):
    ...
    if option == 'Hours of Sleep Per Night for Last 7 Days':
        ...
        explanation = f"The bar chart represents the hours of sleep per night for the last 7 days. The x-axis represents the date, ranging from {plot_df['Date'].min().strftime('%b %d, %Y')} to {plot_df['Date'].max().strftime('%b %d, %Y')}. The y-axis represents the hours of sleep. The highest amount of sleep was on {plot_df.loc[plot_df['Hours of Sleep'].idxmax(), 'Date'].strftime('%b %d, %Y')} with {plot_df['Hours of Sleep'].max()} hours of sleep, and the lowest amount of sleep was on {plot_df.loc[plot_df['Hours of Sleep'].idxmin(), 'Date'].strftime('%b %d, %Y')} with {plot_df['Hours of Sleep'].min()} hours of sleep. This chart helps us understand sleep patterns over the past week."
    ...
    if option == 'Top 5 Activities by Total Duration':
        ...
        # Generate and display the explanation
        explanation = f"The bar chart represents the total duration of the top 5 activities. The x-axis represents different activities: {', '.join(plot_df['Activity'])}. The y-axis represents the total duration of each activity. The activity with the longest total duration is '{plot_df.iloc[0]['Activity']}' with a total duration of {plot_df.iloc[0]['Total Duration']} hours, and the activity with the shortest total duration among the top 5 is '{plot_df.iloc[-1]['Activity']}' with a total duration of {plot_df.iloc[-1]['Total Duration'].round(2)} hours. This chart helps us understand which activities take up most of our time."
        st.write(explanation)
    ...
    if option == 'Energy vs Mood':
        ...
        # Generate and display the explanation
        explanation = f"The scatter plot represents the relationship between energy levels and mood after each activity. The x-axis represents energy levels and the y-axis represents mood. The trendline represents the best fit line for the data, indicating the general pattern of the relationship between energy levels and mood. The regression equation represents this relationship quantitatively. The coefficient of 'Energy Level After' in the regression equation indicates how much 'Mood After' changes for each unit change in 'Energy Level After', holding all other variables constant. The summary statistics provide more information about the regression model, including the R-squared value which indicates how well the model fits the data, and the p-values which can be used to determine the statistical significance of the coefficients."
        st.write(explanation)
