...
def display_plot_and_explanation(option):
    ...
    if option == 'Top 5 Activities by Total Duration':
        ...
        # Generate and display the explanation
        explanation = f"The bar chart represents the total duration of the top 5 activities. The x-axis represents different activities: {', '.join(plot_df['Activity'])}. The y-axis represents the total duration of each activity. The activity with the longest total duration is '{plot_df.iloc[0]['Activity']}' with a total duration of {plot_df.iloc[0]['Total Duration']} hours, and the activity with the shortest total duration among the top 5 is '{plot_df.iloc[-1]['Activity']}' with a total duration of {plot_df.iloc[-1]['Total Duration'].round(2)} hours. This chart helps us understand which activities take up most of my time. I think this shows that I value my sleep a lot, as it is the longest activity. Also, I think I'm quite diligent, having spent a lot of time for school works. But I can still say that there's work life balance, as I still get time for tv shows."
        st.write(explanation)
    ...
    if option == 'Proportion of Time Spent at Top 5 Locations':
        ...
        # Generate and display the explanation
        explanation = f"The pie chart represents the proportion of time spent at the top 5 locations. The locations are: {', '.join(plot_df['Location'])}. Each slice of the pie represents a different location, and the size of the slice represents the proportion of time spent at that location. The largest slice represents the location '{plot_df.iloc[0]['Location']}' where most time is spent, and the smallest slice represents the location '{plot_df.iloc[-1]['Location']}' where least time is spent among the top 5 locations. I think this shows that I prefer to stay at home more than going out, as it is the largest slice of the pie. I also think it's pretty reasonable to have spent the least time in the cinema watching movies among the top 5 locations as it is pretty expensive."
        st.write(explanation)
