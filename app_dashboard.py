...
def display_plot_and_explanation(option):
    ...
    if option == 'Top 5 Activities by Total Duration':
        ...
        # Generate and display the explanation
        explanation = f"The bar chart represents the total duration of the top 5 activities. The x-axis represents different activities: {', '.join(plot_df['Activity'])}. The y-axis represents the total duration of each activity. The activity with the longest total duration is '{plot_df.iloc[0]['Activity']}' with a total duration of {plot_df.iloc[0]['Total Duration']} hours, and the activity with the shortest total duration among the top 5 is '{plot_df.iloc[-1]['Activity']}' with a total duration of {plot_df.iloc[-1]['Total Duration'].round(2)} hours. This chart helps us understand which activities take up most of my time. I think this shows that I value my sleep a lot, as it is the longest activity. I also think that I have pretty great work life balance given I have spent more time on 'watch netflix' than 'do assignments'."
        st.write(explanation)
